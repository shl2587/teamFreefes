    ...
    <tbody>
        <c:forEach var="Rep" items="${Rep}">
            <tr class="reply_list_tr">
                ...
                <td class="Rep_list_board"><a class="Rep_list_board_a contentPreview" href="../../../fes_board/mainboard/${Rep.board}">${Rep.content}</a></td>
...
window.onload = function() {
    var contentPreviews = document.querySelectorAll('.contentPreview');

    contentPreviews.forEach(function(contentLink) {
        var contentText = contentLink.textContent || contentLink.innerText;

        if (contentText.length > 30) {
            contentLink.textContent = contentText.substring(0, 30) + '...';
        }
    });
};
</script>