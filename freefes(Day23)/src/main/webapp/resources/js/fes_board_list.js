...
function updateResultsOnPage() {
	...
	const query_string = $.param(getFormDataFromURL());
...
function update_paging(paging_data, query_string) {
	...
	if (paging_data === null) {
		...
		for (let page_number = paging_data.page_begin; page_number <= paging_data.page_end; page_number++) {
			fes_board_paging.append(`<a class="page-link" href="${cpath}/fes_board/fes_board_list?request_page=${page_number}&` + query_string + `">[${page_number}]</a>`);
		}
